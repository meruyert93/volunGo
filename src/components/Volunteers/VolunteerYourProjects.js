...
function VolunteerYourProjects() {
    ...
    yourProjectComponent = (
      <Grid container textAlign="center">
        <Header textAlign="center" color="grey">
          Your did not add projects yet
        </Header>
      </Grid>
    );
  } else {
    yourProjectComponent = yourProjects.map((project) => {
      return <ItemYourProjects key={project.id} {...project} />;
    });
  }
  ...
  return (
    <Segment basic size="large" padded="very" className="backgroundProject">
      <Grid container>
        <Header as="h1" className="headingText home-section-title ">
          {t('your_projects')}
        </Header>
      </Grid>
      <Grid container>{yourProjectComponent}</Grid>
    </Segment>
  );
}
