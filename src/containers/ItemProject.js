...
function ItemProject({
  ...
  const getDates = (endDate, startDate) => {
    ...
    return `${months[String(arrStartDate[1])]} ${arrStartDate[2]} - ${
      months[String(arrEndDate[1])]
    } ${arrEndDate[2]}`;
  };
    ...
    <Grid.Column mobile={16} tablet={5} computer={5} stretched>
      <Card onDoubleClick={() => chosenHandler(id)} className="projectCard">
        ...
        <Card.Content>
          ...
          <div className="tag tag-upcoming" style={{ marginTop: '10px' }}>
            pending
          </div>
        ...
        <Card.Content>
          <Card.Meta>
            <p className="card-info">
              ...
              />
              {endDate && startDate && ' | ' + getDates(endDate, startDate)}{' '}
              {weekdays && ' | ' + weekdays}{' '}
